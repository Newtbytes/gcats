...
RESOURCES_RESOURCEPACK := $(RESOURCES_RESOURCEPACK_DIR)/${SERVER_NAME}.zip
...
$(RESOURCES_DATAPACK_DIR) $(RESOURCES_RESOURCEPACK_DIR) $(SERVER_DIR):
	mkdir -p $@

$(RESOURCES_RESOURCEPACK) : $(RESOURCES_RESOURCEPACK_DIR) $(RESOURCEPACK)
	cp $(RESOURCEPACK) $@

$(RESOURCES_DATAPACK): $(RESOURCES_DATAPACK_DIR) $(DATAPACK)
	cp $(DATAPACK) $@

$(SERVERPACK) $(MODRINTH_MODPACK): $(PAKKU_SOURCES) | $(RESOURCES_DATAPACK) $(RESOURCES_RESOURCEPACK)
	$(PAKKU) export
